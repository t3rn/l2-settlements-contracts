...
contract avpBatchSubmitter is AccessControlUpgradeable {
    ...
    function claimRefundV2OfBatchEmergency(
        address[] calldata _beneficiary,
        bytes32[] calldata _orderId,
        address[] calldata rewardAsset,
        uint256[] calldata maxReward
    ) public onlyOwnerOrOperator {
        for (uint256 i = 0; i < _orderId.length; ++i) {
            bytes32 payloadHash = escrowGMP.getRemotePaymentPayloadHash(_orderId[i]);
            if (payloadHash == bytes32(uint256(2)) || payloadHash == bytes32(uint256(1))) {
                return;
            }
            escrowGMP.twoifyPayloadHash(_orderId[i]);
            ro.settlePayoutWithFeesCall(maxReward[i], rewardAsset[i], _beneficiary[i], address(ro), 1, _orderId[i]);
        }
    }
    ...
    function claimPayoutBatches(
        ...
        if (operators[msg.sender]) {
            avp.backdoorBatchHash(_batchPayloadHash);
        }
