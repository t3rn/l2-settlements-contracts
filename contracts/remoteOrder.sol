...
contract RemoteOrder is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public openMarketPricer;
    ...
    uint256 public currentProtocolFeeFlat;
    ...
    uint256 public settleGasLimit; // Gas limit for settlement
    uint256 public constant PROTOCOL_FEE_PPM = 1e6; // 100% = 1e6 parts-per-million
    uint256 public constant TX_GAS_CONSUMED_ADD_ON = 36000; // Gas consumed by the transaction itself + return args
    ...
    event SetHalted(bool haltIngress, bool haltEgress);
    ...
    function setHalted(bool _haltIngress, bool _haltEgress) external onlyOwner {
        isIngressHalted = _haltIngress;
        isEgressHalted = _haltEgress;
        emit SetHalted(_haltIngress, _haltEgress);
    }
    ...
    function setCurrentProtocolFee(
        ...
        require(_protocolFee <= PROTOCOL_FEE_PPM, "R0#0");
        require(_protocolFeeFlatLow <= _protocolFeeFlatHigh, "R0#0");
    ...
    function ensureAssetIsSupported(address assetHere) public view returns (bool) {
        return assetHere == address(0) || supportedAssets[assetHere] != 0;
    }
    ...
    function calcProtocolFee(uint256 amount, address asset) public view returns (uint256) {
        return
            ((amount * currentProtocolFee + PROTOCOL_FEE_PPM - 1) / PROTOCOL_FEE_PPM) +
            (asset == address(0) ? currentProtocolFeeFlatLow : currentProtocolFeeFlatHigh);
    }
    ...
    function order(
        ...
        uint256 insurance,
        uint256 maxReward
    ) public payable isIngressOn {
        uint256 startGasConsumed = gasleft();
        uint32 nonce = uint32(block.timestamp);
        bytes32 id = generateId(msg.sender, nonce);
        require(ensureAssetIsSupported(rewardAsset), "RO#2");
        require(ensureDestAssetIsSupported(asset, destination), "RO#2");
        ...
        emit OrderGasConsumed(
            id,
            rewardAsset,
            startGasConsumed - gasleft() + TX_GAS_CONSUMED_ADD_ON,
            maxReward,
            amount
        );
    }
    ...
    function confirmOrderV3(
        ...
        emit ExecuteGasConsumed(id, asset, startGasConsumed - gasleft() + TX_GAS_CONSUMED_ADD_ON);
        return true;
    }
    ...
    function settlePayoutWithFees(uint256 amount, address asset, address beneficiary, address sender) internal {
        uint256 fees = currentProtocolFee > 0 ? calcProtocolFee(amount, asset) : 0;
        if (fees > amount) {
            fees = amount;
        }
    ...
    function settleNativeOrToken(
        ...
        if (asset == address(0)) {
            bool success = false;
            uint256 _settleGasLimit = settleGasLimit == 0 ? 21000 : settleGasLimit;
            assembly {
                success := call(_settleGasLimit, beneficiary, amount, 0, 0, 0, 0)
            }
            return success;
        }
    ...
    function emergencyWithdraw(address asset, uint256 amount, address beneficiary) external onlyOwnerOrFeeCollector {
